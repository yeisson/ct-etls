...
class formatearData(beam.DoFn):
	...
	def process(self, element):
		...
		tupla= {'ID_PREGUNTA' : arrayCSV[0],
                'NOMBRE_PREGUNTA' : arrayCSV[1],
                'ESTADO' : arrayCSV[2],
                'FECHA_MODIF' : arrayCSV[3],
                'USER_MODIF' : arrayCSV[4]
				}
...
def run():
	...
	lines = pipeline | 'Lectura de Archivo' >> ReadFromText(gcs_path + "/evaluacion_desempeno/preguntas" + ".csv")
	...
	transformed | 'Escritura a BigQuery E_D_Preguntas' >> beam.io.WriteToBigQuery(
        gcs_project + ":gestion_humana.E_D_Preguntas",
        schema=TABLE_SCHEMA,
