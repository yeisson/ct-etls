...
class formatearData(beam.DoFn):
	...
	def process(self, element):
		...
		tupla= {'ID_RELCOMPETENCIA' : arrayCSV[0],
                'ID_COMPETENCIA' : arrayCSV[1],
                'ID_PREGUNTA' : arrayCSV[2],
                'ID_TIPO_RESPUESTA' : arrayCSV[3],
                'ESTADO' : arrayCSV[4],
...
def run():
	...
	lines = pipeline | 'Lectura de Archivo' >> ReadFromText(gcs_path + "/evaluacion_desempeno/rel_competencia" + ".csv")
	...
	transformed | 'Escritura a BigQuery E_D_rel_competencia' >> beam.io.WriteToBigQuery(
        gcs_project + ":gestion_humana.E_D_rel_competencia",
        schema=TABLE_SCHEMA,
