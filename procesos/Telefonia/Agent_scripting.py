...
import sys
import requests
import dataflow_pipeline.telefonia.agent_scripting_beam as agent_scripting_beam
# import dataflow_pipeline.telefonia.preview_beam as preview_beam
...
@agent_api.route("/agent", methods=['POST','GET'])
def index():
    ...
    ruta_completa = "/"+ Ruta +"/BI_Archivos/GOOGLE/Telefonia/agent_scripting/" + "Agent_scripting_" + fecha + "_" + operacion + "_" + id_campana +".csv"
    ...
    try:
        QUERY2 = ('delete FROM `contento-bi.telefonia.Agent_scripting` where ipdial_code = ' + '"' + operacion + '"' + 'and (substr(fecha,1,8) between "' + dinip + '" and "'+ dendp + '")' + 'and id_campana = '+ '"' + id_campana + '"')
    ...
    for row in rows:
        ...
        ejecutar = agent_scripting_beam.run(output)
        # time.sleep(180)
